...
├── orchestrator.go
│
├── repo-tl/           ← Git worktree (main)
│
├── repo-agent-1/      ← Git worktree (agent/1/current-task)
│
└── repo-agent-2/      ← Git worktree (agent/2/current-task)
```

Each `repo-*` directory is a separate Git worktree or clone of the same repository.
